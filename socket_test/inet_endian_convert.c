...
{
    printf("192.168.1.1的十六进制表示为0x%X 0x%X 0x%X 0x%X\n",192,168,1,1);
    //声明结构体接收数据
    ...
    //不推荐使用    错误返回-1->表示一个正常可用的输出255.255.255.255
    server_in_addr_t=inet_addr("192.168.1.1");
    printf("inet_addr:0x%X\n",server_in_addr_t);

    //推荐使用 出错返回-1,但传入结构体的数据不是-1
    inet_aton("192.168.1.1",&server_in_addr);
    //具体，仅能使用ip协议
    printf("inet_aton:0x%X\n",server_in_addr.s_addr);

    //万能方法
    inet_pton(AF_INET,"192.168.1.1",&server_in_addr.s_addr);
