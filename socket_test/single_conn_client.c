...
int main(int argc, char const *argv[])
{
    ...
    //填写ip地址0.0.0.0
    inet_pton(AF_INET,"192.168.13.128",&client_addr.sin_addr);
    //填写端口号
    ...
    handle_error("scoket",socketfd);

    //2.绑定
    temp_result=bind(socketfd,(struct sockaddr *)&client_addr,sizeof(client_addr));
    handle_error("bind",temp_result);

    //3.主动连接服务端
    temp_result=connect(socketfd,(struct sockaddr *)&server_addr,sizeof(server_addr));
