...
int main(int argc, char const *argv[])
{
    ...
    //2.绑定地址
    int temp_result=bind(socketfd,(struct sockaddr *)&server_addr,sizeof(server_addr));
    ...
    //此时返回的文件描述符才是能够和客户端收发消息的文件描述符
    //如果调用accept之后没有客户端连接，这里会挂起等待
    int clientfd=accept(socketfd,(struct sockaddr *)&client_addr,&cliaddr_len);
    handle_error("accept",clientfd);

    printf("与客户端%s %d建立连接，文件描述符是%d",inet_ntoa(client_addr.sin_addr),ntohs(client_addr.sin_port),clientfd);
